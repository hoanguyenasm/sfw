...
/*! @brief Defines the PHY AR8031DS vendor defined registers. */
#define PHY_SPECIFIC_STATUS_REG 0x11U /*!< The PHY specific status register. */
#define PHY_PAGE_SELECT_REG     0x1DU /*!< The PHY page select register. */

/*! @brief Defines the PHY AR8031DS ID number. */
#define PHY_CONTROL_ID1 0x004DU /*!< The PHY ID1 . */
...
/*! @brief Defines the PHY AR8031DS extra page and the registers in specified page. */
#define PHY_PAGE_RGMII_TXRX_DELAY_ADDR 0x05U   /*!< The register page including RGMII TX/RX delay setting. */
#define PHY_RGMII_TX_DELAY_REG         0x1EU   /*!< The RGMII TXC delay register. */
#define PHY_RGMII_RX_DELAY_REG         0x1EU   /*!< The RGMII RXC delay register. */
#define PHY_RGMII_TX_DELAY_MASK        0x100U  /*!< The RGMII TXC delay mask. */
#define PHY_RGMII_RX_DELAY_MASK        0x8000U /*!< The RGMII RXC delay mask. */
...
static status_t PHY_AR8031DS_MMD_SetDevice(phy_handle_t *handle,
                                           uint8_t device,
                                           uint16_t addr,
                                           phy_mmd_access_mode_t mode);
static inline status_t PHY_AR8031DS_MMD_ReadData(phy_handle_t *handle, uint32_t *data);
static inline status_t PHY_AR8031DS_MMD_WriteData(phy_handle_t *handle, uint32_t data);
static status_t PHY_AR8031DS_MMD_Read(phy_handle_t *handle, uint8_t device, uint16_t addr, uint32_t *data);
static status_t PHY_AR8031DS_MMD_Write(phy_handle_t *handle, uint8_t device, uint16_t addr, uint32_t data);

/*******************************************************************************
 * Variables
 ******************************************************************************/

const phy_operations_t phyar8031ds_ops = {.phyInit            = PHY_AR8031DS_Init,
                                          .phyWrite           = PHY_AR8031DS_Write,
                                          .phyRead            = PHY_AR8031DS_Read,
                                          .getAutoNegoStatus  = PHY_AR8031DS_GetAutoNegotiationStatus,
                                          .getLinkStatus      = PHY_AR8031DS_GetLinkStatus,
                                          .getLinkSpeedDuplex = PHY_AR8031DS_GetLinkSpeedDuplex,
                                          /*.setLinkSpeedDuplex = PHY_AR8031DS_SetLinkSpeedDuplex,*/
                                          .enableLoopback     = PHY_AR8031DS_EnableLoopback};

/*******************************************************************************
 * Code
 ******************************************************************************/

status_t PHY_AR8031DS_Init(phy_handle_t *handle, const phy_config_t *config)
{
    ...
    {
        /* Get capabilities */
        result = PHY_AR8031DS_MMD_Read(handle, PHY_MDIO_MMD_PCS, PHY_MDIO_PCS_EEE_CAP, &regValue);
        if (result == kStatus_Success)
        {
            /* Enable EEE for 100TX and 1000T */
            result = PHY_AR8031DS_MMD_Write(handle, PHY_MDIO_MMD_AN, PHY_MDIO_AN_EEE_ADV,
...
#endif
    {
        result = PHY_AR8031DS_MMD_Write(handle, PHY_MDIO_MMD_AN, PHY_MDIO_AN_EEE_ADV, 0);
    }
    if (result != kStatus_Success)
    {
        return result;
    ...
    else
    {
        ...
        result = PHY_AR8031DS_SetLinkSpeedDuplex(handle, config->speed, config->duplex);
    }
    return result;
}

status_t PHY_AR8031DS_Write(phy_handle_t *handle, uint32_t phyReg, uint32_t data)
{
    return MDIO_Write(handle->mdioHandle, handle->phyAddr, phyReg, data);
}

status_t PHY_AR8031DS_Read(phy_handle_t *handle, uint32_t phyReg, uint32_t *dataPtr)
{
    return MDIO_Read(handle->mdioHandle, handle->phyAddr, phyReg, dataPtr);
}

status_t PHY_AR8031DS_GetAutoNegotiationStatus(phy_handle_t *handle, bool *status)
{
    assert(status);
...
status_t PHY_AR8031DS_GetLinkStatus(phy_handle_t *handle, bool *status)
{
    assert(status);
...
status_t PHY_AR8031DS_GetLinkSpeedDuplex(phy_handle_t *handle, phy_speed_t *speed, phy_duplex_t *duplex)
{
...
status_t PHY_AR8031DS_SetLinkSpeedDuplex(phy_handle_t *handle, phy_speed_t speed, phy_duplex_t duplex)
{
    status_t result;
    uint32_t regValue;
...
status_t PHY_AR8031DS_EnableLoopback(phy_handle_t *handle, phy_loop_t mode, phy_speed_t speed, bool enable)
{
...
{
    status_t result = kStatus_Success;
    result          = PHY_AR8031DS_MMD_SetDevice(handle, device, addr, kPHY_MMDAccessNoPostIncrement);
    if (result == kStatus_Success)
    {
        result = PHY_AR8031DS_MMD_ReadData(handle, data);
    }
    return result;
}
...
static status_t PHY_AR8031DS_MMD_Write(phy_handle_t *handle, uint8_t device, uint16_t addr, uint32_t data)
{
    ...
    if (result == kStatus_Success)
    {
        result = PHY_AR8031DS_MMD_WriteData(handle, data);
    }
    return result;
}
